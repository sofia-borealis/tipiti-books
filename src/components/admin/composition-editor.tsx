...
export function CompositionEditor({
  ...
  return (
    <div className="space-y-4">
      {/* Row 1: Canvas preview (full width) */}
      <CompositionCanvas
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs transition-colors whitespace-nowrap ${
                    idx === selectedSceneIdx
                      ? 'bg-terracota/10 text-terracota border border-terracota/20 font-semibold'
                      : 'hover:bg-cream text-text-light border border-border-light'
                  }`}
                >
                  #{s.scene_number}
                  {s.background_url && (
          ...
          {variants.length > 0 && (
            <div>
              ...
              <select
                ...
                className="w-full max-w-md rounded-lg border border-border bg-white px-2 py-1.5 text-xs text-text"
              >
                {variants.map((v) => (
